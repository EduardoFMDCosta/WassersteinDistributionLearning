...
def assess_kmeans_feasibility(N, k, dimension=2, max_memory_mb=2000, max_operations=1e9):
    ...
    # # Specific problematic cases based on empirical observation
    # if N > 50000 and k > 1000:
    #     reasons.append(f"Large-scale problem: N={N}, k={k} likely to be very slow")
    
    # # K-means specific issues
    # if N > 10000 and k > 100:
    #     # Distance matrix becomes very large
    #     distance_memory = N * k * 8 / (1024 * 1024)
    #     if distance_memory > max_memory_mb / 2:  # Use half memory limit for distance matrix
    #         reasons.append(f"Distance matrix too large: {distance_memory:.1f}MB")
    
    # # Time-based heuristics (empirical)
    estimated_time_seconds = operations / 1e8  # Rough estimate: 100M operations per second
    # if estimated_time_seconds > 30:  # More than 30 seconds
    #     reasons.append(f"Estimated time: {estimated_time_seconds:.1f}s > 30s")
